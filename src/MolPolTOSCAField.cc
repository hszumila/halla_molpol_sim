...
MolPolTOSCAField::MolPolTOSCAField( G4String filename ){

  G4cout << "Tosca constructor: " << filename << " started..." << G4endl;
...
G4String MolPolTOSCAField::GetName(){
  if( !fInit ){
    G4cerr << "WARNING " << __FILE__ << " line " << __LINE__ << ": access uninitialized field." << G4endl;
    return G4String("");
  }
...
void MolPolTOSCAField::SetMagnetCurrent(G4double s){
  ...
  else G4cerr << "Warning:  " << __FILE__ << " line " << __LINE__ << ": Field current not specified in map " << fFilename << " - Ignoring and proceeding " << G4endl;
  return;
}
...
void MolPolTOSCAField::InitializeGrid() {
  ...
  if( fN[kR] <= 0 || fN[kPhi] <= 0 || fN[kZ] <= 0 ){
    G4cerr << "Error " << __FILE__ << " line " << __LINE__ << ": grid size invalid.  Aborting" << G4endl;
    exit(1);
  }
...
void MolPolTOSCAField::ReadFieldMap(){
  ...
  if (!inputfile.good() ){
    G4cerr << "Error " << __FILE__ << " line " << __LINE__ << ": File " << fFilename << " could not open.  Aborting" << G4endl;
    exit(1);
  }
  ...
  for( cidx = kR; cidx <= kZ; cidx++ ){
    ...
    if (std::istringstream(inputline) >> fN[cidx] >> fMin[cidx] >> fMax[cidx]) {
      G4cout << "N,min,max[" << cidx << "] = " << fN[cidx] << "," << fMin[cidx] << "," << fMax[cidx] << G4endl;
    } else {
      G4cerr << "Error " << __FILE__ << " line " << __LINE__ << ": File " << fFilename << " contains unreadable header.  Aborting" << G4endl;
      exit(1);
    }
  }
  ...
  if (std::istringstream(inputline) >> fPhiMapOffset >> fZMapOffset) {
    G4cout << "PhiMapOffset,ZMapOffset = " << fPhiMapOffset << "," << fZMapOffset << G4endl;
  } else {
    G4cerr << "Error " << __FILE__ << " line " << __LINE__ << ": File " << fFilename << " contains unreadable header.  Aborting" << G4endl;
    exit(1);
  }
  ...
  if( !(fMin[kR] >= 0.0 &&
        ...
        fMin[kPhi] < 180.0 &&
      	...
      	fNxtant > 0) ){
  	G4cerr << "Error " << __FILE__ << " line " << __LINE__ << ": File " << fFilename << " contains invalid header data.  Aborting" << G4endl;
  	exit(1);
  }
  ...
  if( !( fMin[kPhi] >= -pi && fMin[kPhi] <= pi ) ){
    G4cerr << "Error " << __FILE__ << " line " << __LINE__ << ": File " << fFilename
           << " header contains invalid phi range.  Aborting" << G4endl;
    exit(1);
  }

  if( fabs( fabs((mapphirange - fxtantSize)/mapphirange)) > 0.03 ){
    G4cerr << "Warning " << __FILE__ << " line " << __LINE__ << ": File " << fFilename
           << " header contains too narrow phi range for given xtants." << G4endl
           <<  "Warning:   Proceeding assuming null field in non-described regions"
           << G4endl << (fMax[kPhi] - fMin[kPhi])/degree << " deg range < "
           <<  fxtantSize/degree << " deg xtant" << G4endl;
  }
    ...
    for(pidx = 0; pidx < fN[kPhi]; pidx++){
      for(ridx = 0; ridx < fN[kR]; ridx++){
        ...
        if (std::istringstream(inputline) >> raw_R_m >> raw_Phi_deg >> raw_Z_m >> br >> bp >> bz) {
          ...
          G4cerr << "Error " << __FILE__ << " line " << __LINE__ << ": File " << fFilename << " contains invalid data.  Aborting" << G4endl;
          exit(1);
        }
    		...
    		if( ridx == 0 ){
  		    if( fabs(raw_R_m*m - fMin[kR]) > eps ){
      			G4cerr << "Error " << __FILE__ << " line " << __LINE__
      			       ...
      			       << "Index ("<< ridx << ", " << pidx << ", " <<  zidx <<  ")  Expected "
                   << fMin[kR]/m << " read " << raw_R_m << G4endl;
      			exit(1);
  		    }
    		}
    		...
    		if( pidx == 0 ){
  		    if( fabs(raw_Phi_deg*deg - fFileMin[kPhi]) > eps ){
      			G4cerr << "Error " << __FILE__ << " line " << __LINE__
      			       ...
      			       << "Index ("<< ridx << ", " << pidx << ", " <<  zidx <<  ")  Expected "
                   << fFileMin[kPhi]/degree << " read " << raw_Phi_deg << G4endl;
      			exit(1);
  		    }
    		}
    		...
    		if( pidx == fN[kPhi]-1 ){
  		    if( fabs(raw_Phi_deg*deg - fFileMax[kPhi]) > eps ){
      			G4cerr << "Error " << __FILE__ << " line " << __LINE__
      			       << ": File " << fFilename << " contains bad data framing in Phi.  Aborting" << G4endl
      			       << "Index ("<< ridx << ", " << pidx << ", " <<  zidx <<  ")  Expected "
                   << fFileMax[kPhi]/m << " read " << raw_Phi_deg << G4endl;
      			exit(1);
  		    }
    		}
    		...
    		if( zidx == fN[kZ]-1 ){
  		    if( fabs(raw_Z_m*m - fFileMax[kZ]) > eps ){
      			G4cerr << "Error " << __FILE__ << " line " << __LINE__
      			       << ": File " << fFilename << " contains bad data framing in Z.  Aborting" << G4endl
      			       << "Index ("<< ridx << ", " << pidx << ", " <<  zidx <<  ")  Expected "
                   << fFileMax[kZ]/m << " read " << raw_Z_m << G4endl;
      			exit(1);
  		    }
    		}
...
void MolPolTOSCAField::GetFieldValue(const G4double Point[4], G4double *Bfield ) const {
  ...
  if( std::isnan(phi) || std::isinf(phi) || std::isnan(r) || std::isinf(r) || std::isnan(z) || std::isinf(z) ){
  ...
  dphi = phi - fPhiLow >= 0.0 ? modf( (phi - fPhiLow)/fxtantSize, &dxtant ) :
